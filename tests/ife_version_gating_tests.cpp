...
static_assert(vt::FILE_HEADER::offset::RUNTIME_FLAGS == ife_test::FILE_HEADER_PREFIX_SIZE);
static_assert(vt::FILE_HEADER::header_size == vt::FILE_HEADER::header_size_v200_0);
static_assert(vt::LAYER_EXTENTS::entry::offset::RESERVED_EXTENT == ife_test::LAYER_EXTENT_PREFIX_SIZE);
static_assert(vt::LAYER_EXTENTS::entry_size == vt::LAYER_EXTENTS::entry_size_v200_0);
static_assert(vt::FILE_HEADER::header_size == ife_test::FILE_HEADER_HEADER_SIZE);
static_assert(vt::LAYER_EXTENTS::entry_size == ife_test::LAYER_EXTENT_ENTRY_SIZE);
static_assert(vt::FILE_HEADER::offset::RUNTIME_FLAGS == ife_test::RUNTIME_FLAGS_AT);
static_assert(vt::LAYER_EXTENTS::entry::offset::RESERVED_EXTENT == ife_test::RESERVED_EXTENT_AT);
...
constexpr Layout layout(Offset fh_size, std::uint16_t entry_stride, std::uint32_t entry_count = 2) {
    ...
    const Offset le = tt + vt::TILE_TABLE::header_size;
    const Offset to = le + vt::LAYER_EXTENTS::header_size + static_cast<Offset>(entry_count) * entry_stride;
    const Offset md = to + vt::TILE_OFFSETS::header_size + vt::TILE_OFFSETS::entry_size;
    return {tt, le, to, md, md + vt::METADATA::header_size};
}

void store_header(BYTE* p, Offset fh_end, Offset tt, Offset md, std::uint32_t version) {
    ::IFE::store<std::uint32_t>(p + vt::FILE_HEADER::offset::MAGIC, k::MAGIC_BYTES);
    ::IFE::store<std::uint16_t>(p + vt::FILE_HEADER::offset::RECOVERY,
                                static_cast<std::uint16_t>(k::RecoveryCodes::RECOVER_FILE_HEADER));
    ::IFE::store<std::uint64_t>(p + vt::FILE_HEADER::offset::FILE_SIZE, fh_end);
    ::IFE::store<std::uint16_t>(p + vt::FILE_HEADER::offset::EXTENSION_MAJOR,
                                static_cast<std::uint16_t>(version >> 16));
    ::IFE::store<std::uint16_t>(p + vt::FILE_HEADER::offset::EXTENSION_MINOR,
                                static_cast<std::uint16_t>(version & 0xFFFF));
    ::IFE::store<std::uint32_t>(p + vt::FILE_HEADER::offset::FILE_REVISION, 7);
    ::IFE::store<std::uint64_t>(p + vt::FILE_HEADER::offset::TILE_TABLE_OFFSET, tt);
    ::IFE::store<std::uint64_t>(p + vt::FILE_HEADER::offset::METADATA_OFFSET, md);
}

void store_tile_table(BYTE* p, Offset at, Offset le, Offset to) {
    ::IFE::store<std::uint64_t>(p + at + vt::BLOCK::offset::VALIDATION, at);
    ::IFE::store<std::uint16_t>(p + at + vt::BLOCK::offset::RECOVERY,
                                static_cast<std::uint16_t>(k::RecoveryCodes::RECOVER_TILE_TABLE));
    ::IFE::store<std::uint8_t>(p + at + vt::TILE_TABLE::offset::ENCODING,
                               static_cast<std::uint8_t>(k::TileEncodings::TILE_ENCODING_JPEG));
    ::IFE::store<std::uint8_t>(p + at + vt::TILE_TABLE::offset::FORMAT,
                               static_cast<std::uint8_t>(k::PixelFormats::FORMAT_R8G8B8A8));
    ::IFE::store<std::uint64_t>(p + at + vt::TILE_TABLE::offset::CIPHER_OFFSET, k::NULL_OFFSET);
    ::IFE::store<std::uint64_t>(p + at + vt::TILE_TABLE::offset::TILE_OFFSETS_OFFSET, to);
    ::IFE::store<std::uint64_t>(p + at + vt::TILE_TABLE::offset::LAYER_EXTENTS_OFFSET, le);
    ::IFE::store<std::uint32_t>(p + at + vt::TILE_TABLE::offset::X_EXTENT, 4096);
    ::IFE::store<std::uint32_t>(p + at + vt::TILE_TABLE::offset::Y_EXTENT, 2048);
}
...
void store_metadata(BYTE* p, Offset at) {
    ...
    ::IFE::store<std::uint16_t>(p + at + vt::METADATA::offset::CODEC_MAJOR, 2);
...
std::vector<BYTE> make_file(std::uint32_t version, Offset fh_size, std::uint16_t entry_stride,
    ...
    if (with_200_fields)
        ::IFE::store<std::uint32_t>(p + vt::FILE_HEADER::offset::RUNTIME_FLAGS, RUNTIME_FLAGS_VALUE);

    store_tile_table(p, L.tile_table, L.layer_extents, L.tile_offsets);

    ::IFE::store<std::uint64_t>(p + L.layer_extents + vt::ARRAY::offset::VALIDATION, L.layer_extents);
    ::IFE::store<std::uint16_t>(p + L.layer_extents + vt::ARRAY::offset::RECOVERY,
                                static_cast<std::uint16_t>(k::RecoveryCodes::RECOVER_LAYER_EXTENTS));
    ::IFE::store<std::uint16_t>(p + L.layer_extents + vt::ARRAY::offset::STRIDE, entry_stride);
    ::IFE::store<std::uint32_t>(p + L.layer_extents + vt::ARRAY::offset::COUNT, entry_count);
    for (std::uint32_t i = 0; i < entry_count; ++i) {
        BYTE* e = p + L.layer_extents + vt::LAYER_EXTENTS::header_size + static_cast<Offset>(i) * entry_stride;
        ::IFE::store<std::uint32_t>(e + vt::LAYER_EXTENTS::entry::offset::X_TILES, 8u << i);
        ::IFE::store<std::uint32_t>(e + vt::LAYER_EXTENTS::entry::offset::Y_TILES, 8);
        ::IFE::store<float>(e + vt::LAYER_EXTENTS::entry::offset::SCALE, static_cast<float>(i + 1));
        if (with_200_fields)
            ::IFE::store<std::uint16_t>(e + vt::LAYER_EXTENTS::entry::offset::RESERVED_EXTENT,
                                        static_cast<std::uint16_t>(RESERVED_EXTENT_VALUE + i));
    }

    ::IFE::store<std::uint64_t>(p + L.tile_offsets + vt::ARRAY::offset::VALIDATION, L.tile_offsets);
    ::IFE::store<std::uint16_t>(p + L.tile_offsets + vt::ARRAY::offset::RECOVERY,
                                static_cast<std::uint16_t>(k::RecoveryCodes::RECOVER_TILE_OFFSETS));
    ::IFE::store<std::uint16_t>(p + L.tile_offsets + vt::ARRAY::offset::STRIDE, vt::TILE_OFFSETS::entry_size);
    ::IFE::store<std::uint32_t>(p + L.tile_offsets + vt::ARRAY::offset::COUNT, 1);
    BYTE* te = p + L.tile_offsets + vt::TILE_OFFSETS::header_size;
...
void test_1_0_file_read_by_200_0_build() {
    ...
    // other source.
    const auto f = make_file(V1_0, vt::FILE_HEADER::header_size_v1_0, vt::LAYER_EXTENTS::entry_size_v1_0, false);
...
void test_narrow_stride_gates_a_field_the_version_allows() {
    ...
    const auto f = make_file(V200_0, vt::FILE_HEADER::header_size,
                             vt::LAYER_EXTENTS::entry_size_v1_0, false);
    const auto h = root(f, V200_0);

    const auto le = h.tile_table_offset().layer_extents_offset();
    IFE_CHECK(le.stride() == vt::LAYER_EXTENTS::entry_size_v1_0);
    IFE_CHECK(le.stride() < vt::LAYER_EXTENTS::entry_size);   // narrower than compiled
    IFE_CHECK(le.entry(0).reserved_extent() == std::nullopt);
...
void test_200_0_file_read_by_200_0_build() {
    const auto f = make_file(V200_0, vt::FILE_HEADER::header_size, vt::LAYER_EXTENTS::entry_size, true);
    const auto h = root(f, V200_0);
...
void test_wide_stride_is_stepped_and_tail_skipped() {
    ...
    const std::uint16_t wide = static_cast<std::uint16_t>(vt::LAYER_EXTENTS::entry_size + 4);
    const auto f = make_file(V200_0, vt::FILE_HEADER::header_size, wide, true);
...
void test_newer_minor_reads_without_error() {
    ...
    const auto f = make_file(V200_1, vt::FILE_HEADER::header_size, vt::LAYER_EXTENTS::entry_size, true);
